...
map<string,pair<long long,string> > names;
struct num_to_do{bool type;string a;int b;}nums[10000][2000];
long long ttime=1763124500;
string s[1000000];
bool is_in,is_daoru,a[1000][2000];
...
void error(){cout<<"error!you fucked YOU by a wrong way!\n";while(1);}
void dosth(string x,int nowif)
{
	...
	if(x=="end")
	{
		if(a[nowif][0]==1)
		{
			if(nums[nowif][0].type==1)error();
			if(nums[nowif][1].type==1)error();
		...
		else if(a[nowif][7]==1)
		{
			...
			if(nums[nowif][1].type==0)error();
			if(nums[nowif][2].type==0)error();	
			if(nums[nowif][0].b==0)return;
			int cntend=0; 
			for(i2[nowif]=0;cntend<nums[nowif][1].b-1&&i2[nowif]<cntcode-1;i2[nowif]++)
			{
				if(s[i2[nowif]]=="YOU!")cntend++;
			}
			for(;cntend<nums[nowif][2].b&&i2[nowif]<cntcode-1;i2[nowif]++)
			{
				if(s[i2[nowif]]=="YOU!")
				{
					dosth("end",nowif+1);
...
int main()
{
	names["cuizihangya"]={1e9,"12345678"};
	names["flat_fat_cat"]={1e9,"ffckcuf"};
	cout<<"是否登录？\n";
	cout<<"是 1，否 0\n";
	cout<<"登录可享代码保存权及运行已保存代码权利\n";
	int temp;cin>>temp;
	if(temp==1)
	{
		cout<<"输入账号密码：账号 密码\n"; 
		...
		while(1)
		{
			...
			if(names.count(x))
			{
				if(names[x].second==y)
				{
					if((time(0)-ttime)/60/60/24>names[x].first)
					{
						cout<<"VIP已过期，请联系@cuizihang\n";
						return 0;
					}
					break;
				}
		...
		cout<<"登录成功，是否运行\n是 1，否 0\n";
		cin>>temp;
		if(temp==1)is_daoru=1,freopen("code.txt","r",stdin);
		is_in=1;
	}
	if(is_daoru==0)cout<<"开始输入代码：\n";
	string p;
	while(1)
	{
		cin>>s[cntcode];
	...
	if(is_in&&!is_daoru)
	{
		...
		if(temp==1)
		{
			...
			for(int i=0;i<cntcode;i++)cout<<s[i]<<" ";
		}
